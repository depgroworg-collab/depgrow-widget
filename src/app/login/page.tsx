  ...
  return (
    <div style={{minHeight:'100vh',background:'#f9fafb',display:'flex',alignItems:'center',justifyContent:'center',padding:'2rem',fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif'}}>
      <div style={{width:'100%',maxWidth:440,background:'#fff',borderRadius:20,padding:'2.5rem',boxShadow:'0 4px 32px rgba(0,0,0,0.08)',border:'1px solid #E5E7EB'}}>
        ...
        <h1 style={{fontSize:26,fontWeight:800,color:'#111827',marginBottom:6,letterSpacing:-0.5}}>Welcome back</h1>
        ...
        <form onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:'1.25rem'}}>
          {[
            ...
            <div key={f.label}>
              ...
              <div style={{border:'1.5px solid #E5E7EB',borderRadius:10,background:'#fff',transition:'border-color 0.15s'}}>
                <input
                  type={f.type}
                  placeholder={f.ph}
                  value={f.val}
        ...
        <p style={{textAlign:'center',fontSize:13,color:'#6B7280',marginTop:'1.5rem'}}>
          No account?{' '}
          <Link href="/register" style={{color:'#0E7A5A',fontWeight:600,textDecoration:'none'}}>Create one free</Link>
        </p>
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#f9fafb'}}><div style={{color:'#0E7A5A',fontSize:14}}>Loading…</div></div>}>
      <LoginInner />
    </Suspense>
  )
}
